    ...
    override def render(shapeRenderer: ShapeRenderer): Unit = {
        ...
        if(moving) {
            if (Gdx.input.isKeyPressed(Input.Keys.W)) {
                ...
                // speed squared, which is the original speed. The square root is then simplified
                if (Gdx.input.isKeyPressed(Input.Keys.A) || Gdx.input.isKeyPressed(Input.Keys.D)) getComponent(classOf[RectComponent]).move(new Vector2(0, speed / sqrt(2).toFloat))
                // Go at full speed when moving normally
                else getComponent(classOf[RectComponent]).move(new Vector2(0, speed))
            }
            if (Gdx.input.isKeyPressed(Input.Keys.A)) {
                if (Gdx.input.isKeyPressed(Input.Keys.W) || Gdx.input.isKeyPressed(Input.Keys.S)) getComponent(classOf[RectComponent]).move(new Vector2(-speed / sqrt(2).toFloat, 0))
